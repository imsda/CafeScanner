...
function PeoplePage() {
  ...
  async function savePerson(person: PersonRecord) {
    ...
    setMessage('');
    const payload = isTally
      ? { breakfastCount: person.breakfastCount, lunchCount: person.lunchCount, dinnerCount: person.dinnerCount, totalMealsCount: person.breakfastCount + person.lunchCount + person.dinnerCount }
...
function SettingsPage() {
  ...
  const [clearAction, setClearAction] = useState<'clear-meal-data' | 'clear-people-import-data' | 'reset-meal-tracking-data'>('reset-meal-tracking-data');
  ...
  const [fullWipeResult, setFullWipeResult] = useState<{ token: string; expiresAt: string } | null>(null);
  ...
  const clearEnabled = clearPhrase === 'RESET MEAL TRACKING DATA' && !isClearingDatabase;
  ...
  async function armFullWipe() {
    ...
    setError('');
    setMessage('');
    const response = await api<{ token: string; expiresAt: string }>('/settings/full-wipe/arm', { method: 'POST', body: JSON.stringify({ confirmationPhrase: fullWipePhrase }) });
    setFullWipeResult(response);
    setShowFullWipeConfirm(false);
    setFullWipePhrase('');
  }

  return <div className="card stack"><h2>Settings</h2><div className="card stack"><h3>Meal Tracking Mode</h3><p className="muted">Current active mode: <strong>{modeLabel(settings.mealTrackingMode)}</strong></p><label>Meal tracking mode<select value={settings.mealTrackingMode} onChange={(e) => { const selected = e.target.value as MealTrackingMode; if (selected === settings.mealTrackingMode) return; setPendingMode(selected); setShowModeConfirm(true); setModePhrase(''); }}><option value="camp_meeting">Camp Meeting (redeem imported meal entitlements)</option><option value="countdown">Count Down (deduct from remaining balances)</option><option value="tally">Tally Up (count each served meal)</option></select></label><p className="error">Warning: Switching mode is destructive and will clear all people, transaction history, import history, and meal entitlements.</p>{showModeConfirm && pendingMode && <div className="confirm-overlay" role="dialog" aria-modal="true"><div className="confirm-modal stack"><h4>Confirm Mode Switch</h4><p>You are switching from <strong>{modeLabel(settings.mealTrackingMode)}</strong> to <strong>{modeLabel(pendingMode)}</strong>.</p><p className="error">This will permanently clear operational data (people, scans, imports). Accounts and settings will be preserved.</p><p>Type <code>SWITCH MODE</code> to continue.</p><input value={modePhrase} onChange={(e) => setModePhrase(e.target.value)} placeholder="SWITCH MODE" /><div className="button-row"><button className="secondary" type="button" onClick={() => { setShowModeConfirm(false); setPendingMode(null); setModePhrase(''); }}>Cancel</button><button className="danger" type="button" disabled={!modeEnabled} onClick={() => void api('/settings/meal-tracking-mode', { method: 'PUT', body: JSON.stringify({ mealTrackingMode: pendingMode, confirmationPhrase: modePhrase }) }).then(async () => { setMessage(`Meal tracking mode switched to ${modeLabel(pendingMode)}. Operational data was cleared.`); setError(''); setShowModeConfirm(false); setPendingMode(null); setModePhrase(''); await load(); })}>Switch Mode + Clear Data</button></div></div></div>}</div><div className="card stack"><h3>Scanner</h3><label>Scan cooldown<input type="number" min={0.5} max={10} step={0.1} value={settings.scannerCooldownSeconds} onChange={(e) => setSettings({ ...settings, scannerCooldownSeconds: Math.min(10, Math.max(0.5, Number(e.target.value) || 1)) })} /></label><p className="muted">Seconds between duplicate scans of the same ID.</p><label><input type="checkbox" checked={settings.scannerDiagnosticsEnabled} onChange={(e) => setSettings({ ...settings, scannerDiagnosticsEnabled: e.target.checked })} /> Enable scanner diagnostics</label><p className="muted">Shows technical camera/scanner information for troubleshooting.</p></div><div className="grid-form">{Object.keys(settings).filter((k)=>!['id','updatedAt','mealTrackingMode','scannerCooldownSeconds','scannerDiagnosticsEnabled'].includes(k)).map((k)=><label key={k}>{k}<input value={String(settings[k as keyof Settings])} onChange={(e)=>setSettings({...settings,[k]:typeof settings[k as keyof Settings]==='boolean'?e.target.value==='true':typeof settings[k as keyof Settings]==='number'?Number(e.target.value):e.target.value})}/></label>)}</div><div className="button-row"><button className="primary" onClick={() => void saveSettings()}>Save</button></div>{message && <p>{message}</p>}{error && <p className="error">{error}</p>}<hr /><div className="stack"><h3>System: Data Reset Tools</h3><p className="error">Choose the scope carefully. These actions only clear meal tracking operational data and never remove users, passwords, roles, enabled/disabled status, or page permissions.</p><button className="danger" type="button" onClick={() => { setShowClearModal(true); setMessage(''); setError(''); setClearPhrase(''); }}>Open Reset Options</button>{showClearModal && <div className="confirm-overlay" role="dialog" aria-modal="true"><div className="confirm-modal stack"><h4>Reset Meal Tracking Data</h4><p className="error"><strong>Warning:</strong> This action only removes meal tracking operational data. Accounts and permissions are preserved.</p><label>Reset action<select value={clearAction} onChange={(e) => setClearAction(e.target.value as 'clear-meal-data' | 'clear-people-import-data' | 'reset-meal-tracking-data')}><option value="clear-meal-data">Clear Meal Data</option><option value="clear-people-import-data">Clear People/Import Data</option><option value="reset-meal-tracking-data">Reset Meal Tracking Data</option></select></label><p>Type <code>RESET MEAL TRACKING DATA</code> to continue.</p><input value={clearPhrase} onChange={(e) => setClearPhrase(e.target.value)} placeholder="RESET MEAL TRACKING DATA" /><div className="button-row"><button className="secondary" onClick={() => { setShowClearModal(false); setClearPhrase(''); }} disabled={isClearingDatabase}>Cancel</button><button className="danger" disabled={!clearEnabled} onClick={() => void clearOperationalData()}>{isClearingDatabase ? 'Running…' : 'Confirm Selected Reset'}</button></div></div></div>}</div>{isOwner && (<><hr /><div className="stack"><h3>Full Application Wipe</h3><p className="error">This permanently deletes the database, including users, settings, people, imports, transactions, and permissions. This cannot be undone.</p><p className="muted">This page can only arm the wipe token. The deletion can only be executed from the terminal script.</p><button className="danger" type="button" onClick={() => { setShowFullWipeConfirm(true); setFullWipePhrase(''); setFullWipeResult(null); setMessage(''); setError(''); }}>Arm Full Application Wipe</button>{showFullWipeConfirm && <div className="confirm-overlay" role="dialog" aria-modal="true"><div className="confirm-modal stack"><h4>Arm Full Application Wipe</h4><p className="error">Type <code>ARM FULL WIPE</code> to arm a one-time terminal wipe token.</p><input value={fullWipePhrase} onChange={(e) => setFullWipePhrase(e.target.value)} placeholder="ARM FULL WIPE" /><div className="button-row"><button className="secondary" onClick={() => { setShowFullWipeConfirm(false); setFullWipePhrase(''); }}>Cancel</button><button className="danger" disabled={!fullWipeEnabled} onClick={() => void armFullWipe()}>Arm Full Application Wipe</button></div></div></div>}{fullWipeResult && <div className="card stack"><h4>Full Wipe Token (shown once)</h4><p className="error">Token expires in 15 minutes.</p><p><code>{fullWipeResult.token}</code></p><p>Run this command in the terminal:</p><p><code>{'npm run full-wipe -w backend -- --token ' + fullWipeResult.token}</code></p><p className="muted">Do not close this page until you have copied the token. It is not stored in local storage and will not be shown again.</p></div>}</div></>)}</div></div>;
}
...
function UserManagementPage() {
  ...
  return <div className="card stack"><h2>User Management</h2>
    ...
    <table><thead><tr><th>User</th><th>Role</th><th>Allowed Pages</th><th>Actions</th></tr></thead><tbody>
      {users.map((u) => <tr key={u.id}><td>{u.username}</td><td>{u.role}</td><td>{formatPages((u.allowedPages || []) as AppPage[])}</td><td><button className="secondary" onClick={() => { const next = prompt(`New password for ${u.username}`); if (next) void api(`/users/${u.id}`, { method: 'PATCH', body: JSON.stringify({ password: next }) }).then(loadUsers); }}>Reset Password</button> <button className="secondary" onClick={() => { if (confirm(`Delete ${u.username}?`)) void api(`/users/${u.id}`, { method: 'DELETE' }).then(loadUsers); }}>Delete</button></td></tr>)}
    </tbody></table>
  </div>;
}
